...
export default class TicketController {
    ...
    async categorizeTicket(req, res) {
        try {
            ...
            if (!task_id) {
                return res.status(422).json({ success: false, message: 'task_id not provided' });
            }
            ...
            if (isFromOurAPI) {
                return res.status(200).json({ success: true, message: 'Event from own API ignored' });
            }
            
            if (!history_items.some(item => item.user)) {
                return res.status(422).json({ success: false, message: 'event not generated by a user' });
            }
            
            let cacheKey = `ticket_${task_id}`;
            if (await this.redisService.has(cacheKey)) {
                return res.status(200).json({ success: true, message: 'ticket already processed recently' });
            }
            ...
            if (isAlreadyProcessed) {
                console.log('Ticket already categorized:', task_id);
            ...
            if (!categorization) {
                console.warn('Could not categorize ticket:', task_id);
                return;
            }
